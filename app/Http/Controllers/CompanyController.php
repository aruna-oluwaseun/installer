...
class CompanyController extends Controller
{
    ...
    public function index($company_id)
    {
        $company = Company::with(['reviews' => function($q) {
            $q->active();
        },'services','media'])->findOrFail($company_id);

        // record stat
        CompanyStat::record($company_id,'profile');

        set_active_menu('find-installer');
        set_page_title($company->title);
        return view('business',compact(['company']));
    }
    ...
    public function edit()
    {
        ...
        //dd($company);
        set_active_menu('company/profile');
        set_page_title('Manage your company profile');
        return view('account.company-profile', compact(['company']));
    }
